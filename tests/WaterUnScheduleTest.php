...
class WaterUnScheduleTest extends TestCase
{
    ...
    {
        $response = air_selangor()->unscheduled()->fetch();
    ...
    {
        $response = air_selangor()->getList()->unscheduled()->byDistrict("00")->fetch();
        $this->assertFalse($response['error']);
        $this->assertEquals(200, $response['code']);
    }
    ...
    {
        $response = air_selangor()->getList()->unscheduled()->byDistrict("900")->fetch();
        $this->assertTrue($response['error']);
        $this->assertEquals(400, $response['code']);

        $response = air_selangor()->unscheduled()->byDistrict("00")->fetch();
        $this->assertTrue($response['error']);
        $this->assertEquals(400, $response['code']);
    }
}